window.onload = async function () {
  ...
  try {
    ...
    if (dirPath) {
      ...
      files = files.filter((file) => {
        ...
        if (!shouldKeep) {
          console.log(
            `Removing: ${file} (Reason: ${
              fileName === ".DS_Store" ? "DS_Store file" : "Blocked file type"
            })`
          );
        }
  ...
  async function handleRename() {
    ...
    const originalExtension = currentFile.substring(
    ...
    try {
      ...
      const allFileNames = allFilePaths.map((filePath) =>
        window.file.pathBasename(filePath)
      );
      ...
      if (response.success) {
        showNotification(
          `File renamed successfully to ${finalName}`,
          "success"
        );
  ...
  function containsIllegalCharacters(name) {
    ...
    if (isMac && (illegalMacLinux.test(name) || illegalMac.test(name)))
      return true;
  ...
  async function refreshPreview() {
    ...
    previewContainer.innerHTML = await window.file.generatePreviewHTML(
      filename
    );
  ...
  function moveSwipe(e) {
    ...
    previewContainer.style.transform = `translateX(${diffX}px) rotate(${
      diffX / 15
    }deg)`;
  }
  ...
  function LLM() {
    ...
    if (mimeType && mimeType.startsWith("image/")) {
      try {
        // Read the file and directly obtain a Base64-encoded string.
        ...
        window.openai
          ...
          .catch((error) => {
            ...
            popupContent.textContent =
              "This image goes against our requirements.";
          });
      } catch (error) {
