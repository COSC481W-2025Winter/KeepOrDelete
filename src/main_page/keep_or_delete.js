...
window.onload = async function () {
    ...
    sortOrderDropdown.addEventListener("change", () => {
        ...
        if (sortOrder === "asc" || sortOrder === "desc") { //id's for name sorting
            sortFilesByName(sortOrder);
        } else if (sortOrder === "up" || sortOrder === "down") { //id's for size sorting
            sortFilesBySize(sortOrder);
        }
        displayCurrentFile(); //re render
    });


    //function for name- stelas code
    ...
    function sortFilesBySize(order) {
        fileObjects.sort((a, b) => {
            return order === "up"
                ? a.size - b.size  //small to big
                : b.size - a.size; //big to small
        });
